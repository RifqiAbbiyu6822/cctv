...
class CarCounter:
    ...
    def process_frame(self, frame, tracking=True, confidence=0.25, iou=0.45):
        ...
        # Update counts untuk UI: gunakan kunci Jalur A/B
        legacy_counts = {
            'mobil': self.counts['total'],
            'Jalur A': self.counts['up'],   # Naik -> Jalur A
            'Jalur B': self.counts['down']  # Turun -> Jalur B
        }
    ...
    def draw_counter_info(self, frame):
        ...
        cv2.putText(frame, f"Jalur A: {self.counts['up']} | Jalur B: {self.counts['down']}", 
